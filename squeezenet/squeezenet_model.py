...
val_loss_list = []
best_val_acc = 0.0
# best_model_path is already defined above
...
for epoch in range(1, EPOCHS + 1):
    ...
    # Save checkpoint each epoch (optional; comment out if not wanted)
...
# -------------------------
# Save final PyTorch state (best) if not already saved
# -------------------------
if not os.path.exists(best_model_path):
    torch.save(model.state_dict(), best_model_path)
    print("Saved best PyTorch model to", best_model_path)
else:
    print("Best PyTorch model available at", best_model_path)

# -------------------------
# Export to ONNX -> SavedModel -> .tflite
# -------------------------
# All paths are already defined at the top of the file
print("\n=== Starting export: PyTorch -> ONNX -> SavedModel -> TFLite ===")

# Load best model weights into model (cpu) for export
...
print("\n=== EXPORT COMPLETE ===")
print("Files created in:", MODEL_SAVE_DIR)
print(" - best PyTorch model:", best_model_path)
